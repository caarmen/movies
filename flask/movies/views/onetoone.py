...
bp = Blueprint("onetoone", __name__, url_prefix="/onetoone")
...
async def async_optim():
    async with get_async_session() as session:
        movies = (
            await session.scalars(select(Movie).options(joinedload(Movie.finance)))
        ).all()
        return render_template(
            "movies/onetoone.html",
            movie_list=movies,
...
def sync_optim():
    movies = db.session.scalars(select(Movie).options(joinedload(Movie.finance))).all()
    return render_template(
        "movies/onetoone.html",
        movie_list=movies,
    )
